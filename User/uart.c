...
void Main_Send_Process(void){
	...
	if(send->buffer_get_index != send->buffer_put_index){
		TX_ENABLE();
...
void MakeTxPacket(BYTE bDataLen, BYTE *bData){
	...
	buffer->data[put_count++] = bin_to_ascii((lrc>>4) & 0x0f);
	buffer->data[put_count++] = bin_to_ascii((lrc & 0x0f));
		...
		if(draw_status.status_list.bit.init_cmpl == true){
			if(((draw_status.status_list.bit.active == true) || (draw_status.status_list.bit.manual == true))
				...
				if(draw_status.status_list.bit.active_lock == false){
					if(b >= draw_encoder.draw_final_param)		b = DRAW_PARAM_OPEN;		//draw_encoder.draw_final_param == DRAW_PARAM_OPEN
...
#ifdef DEBUG
#ifdef DEBUG_UART
	printf("Request Calibration Reset!!\r\n");
#endif
#endif
}
...
void ReqDisplayDraw(void)
{
	...
	if(Display.step != DISPLAY_SETTING){
		...
		draw_status.err_cst_count = (receive->length.W - 1) / 2;
...
void ReceiveFaultCstSlot(void)
{
	...
	if(b >= draw_encoder.draw_final_param)		b = DRAW_PARAM_OPEN;		//exception
...
void ReqEncoder(void)
{
	...
	for(BYTE i=0; i<=draw_encoder.draw_final_param; i++){
		send->payload[cnt++] = ((draw_encoder.draw_enc_point[i] >> 8) & 0xff);
		send->payload[cnt++] |= (draw_encoder.draw_enc_point[i] & 0xff);
	}
	MakeTxPacket(cnt, send->payload);
}
