...
#endif
	temp = 	(((GPIOI->IDR & GPIO_Pin_11) == RESET)<<1) |
			(((GPIOC->IDR & GPIO_Pin_13) == RESET));
...
void MyLocation(void)
{
	static WORD lastEnc = 0xffff;
		...
		if(draw_order.my_location != DRAW_PARAM_UNKNOWN){
			switch(draw_order.my_location){
				case DRAW_PARAM_CLOSE:
					...
					break;
				case DRAW_PARAM_01:
					if(draw_encoder.draw_my_encoder >= draw_encoder.draw_enc_point[DRAW_PARAM_02]){
				...
				case DRAW_PARAM_08:
				case DRAW_PARAM_09:
					if(draw_encoder.draw_my_encoder >= draw_encoder.draw_enc_point[draw_order.my_location+1]){
						draw_order.my_location = draw_order.my_location+1;
					}else if(draw_encoder.draw_my_encoder < draw_encoder.draw_enc_point[draw_order.my_location-1]){
						draw_order.my_location = draw_order.my_location-1;
					}
					break;
				case DRAW_PARAM_10:
				...
				case DRAW_PARAM_17:
				case DRAW_PARAM_18:
					if(draw_order.my_location == draw_encoder.draw_final_param){
						if(draw_encoder.draw_my_encoder <= draw_encoder.draw_enc_point[draw_order.my_location-1]){
							draw_order.my_location = draw_order.my_location-1;
						}
					}else{
				...
				case DRAW_PARAM_OPEN:
					if(draw_encoder.draw_my_encoder <= draw_encoder.draw_enc_point[draw_encoder.draw_final_param]){
						draw_order.my_location = draw_encoder.draw_final_param;
					}
					break;
			}
		}
...
void GetDrawEncoder(void){
	...
	switch(encoder_direction_arry[prev_encoder_status][encoder_status]){
		case DRAW_TACHO_CW:
			if(Display.step == DISPLAY_SETTING)				draw_encoder.draw_tacho_cw++;
			draw_encoder.draw_my_encoder++;
		break;
		case DRAW_TACHO_CCW:
			if(Display.step == DISPLAY_SETTING)				draw_encoder.draw_tacho_ccw++;
			if(draw_encoder.draw_my_encoder > 0)			draw_encoder.draw_my_encoder--;
		break;
	}
	...
	if(draw_order.my_location != DRAW_PARAM_UNKNOWN){
		if(draw_order.order_list.bit.remote_order == true){
			if(draw_encoder.draw_my_encoder == draw_encoder.draw_enc_point[draw_order.order_location]){
			...
			if((draw_encoder.draw_my_encoder == draw_encoder.draw_dec_encoder) && (pwm_status.pwm_list.bit.deceleration == false)){
#ifdef DEBUG
#ifdef DEBUG_ENCODER
				printf("Open Sensor Deceleration![%d]\r\n",draw_encoder.draw_dec_encoder);
#endif
#endif
...
void DrawerAccRatioProc(void)
{
	...
	if(draw_order.my_location == DRAW_PARAM_UNKNOWN){
		draw_encoder.overall_range = 1000;
		pwm_status.pwm_list.bit.slow_section = true;
		draw_encoder.unit = (draw_encoder.overall_range / 100);
	}else{
		switch(draw_order.order_location){
			case DRAW_PARAM_OPEN:
				draw_encoder.overall_range = 1000 - draw_encoder.draw_enc_point[draw_order.my_location];
				break;
			case DRAW_PARAM_CLOSE:
				if(draw_encoder.draw_my_encoder != 0)	draw_encoder.overall_range = draw_encoder.draw_my_encoder;
				break;
			case DRAW_PARAM_01:
			case DRAW_PARAM_02:
			case DRAW_PARAM_03:
	...
	if(draw_encoder.draw_my_encoder < draw_encoder.draw_enc_point[draw_order.order_location]){
		if(draw_order.order_location == DRAW_PARAM_OPEN)	draw_encoder.draw_dec_encoder = 1000 - draw_encoder.acc_dec_section + (draw_encoder.acc_dec_section / 3);
		else												draw_encoder.draw_dec_encoder = draw_encoder.draw_enc_point[draw_order.order_location] - draw_encoder.acc_dec_section + (draw_encoder.acc_dec_section / 3);
	}else{
		draw_encoder.draw_dec_encoder = draw_encoder.draw_enc_point[draw_order.order_location] + draw_encoder.acc_dec_section - (draw_encoder.acc_dec_section / 3);
	}
#ifdef DEBUG
	...
	if(draw_order.loop_order == true){
		...
		switch(draw_order.loop_step){
			case 0:
				switch(draw_order.my_location){
					case DRAW_PARAM_CLOSE:
						if(last_pos < draw_encoder.draw_final_param)		last_pos++;
						else												last_pos = DRAW_PARAM_01;

						if(last_pos == draw_encoder.draw_final_param)		last_pos = DRAW_PARAM_OPEN;		//draw_encoder.draw_final_param == DRAW_PARAM_OPEN

						draw_order.order_location = last_pos;
					...
					case DRAW_PARAM_UNKNOWN:
						printf("%2d %2d ",draw_order.loop_cnt,draw_order.my_location);
			...
			case 1:
				...
				}else if(draw_encoder.draw_my_encoder > draw_encoder.draw_enc_point[draw_order.order_location]){
					DrawerAccRatioProc();
			...
			case 2:
				if(draw_order.order_list.bit.remote_order == false){
					if(draw_order.order_location == DRAW_PARAM_CLOSE)		draw_order.loop_step = 4;
					else													draw_order.loop_step = 3;
					last_time = _tick_count;
				}
				...
				if((draw_status.status_list.bit.timeout) || (draw_status.status_list.bit.encoder_error)){
					...
					//if(draw_status.status_list.bit.timeout)					printf("Timeout[%3d]\r\n",draw_encoder.draw_my_encoder);
					//if(draw_status.status_list.bit.encoder_error)			printf("Encoder Error[%3d]\r\n",draw_encoder.draw_my_encoder);
				}
				break;
			case 3:
				if(GetElapsedTickCount(last_time) >= 1000){
					draw_order.order_list.bit.stop_order = true;
					//printf("%3d ",draw_encoder.draw_my_encoder);
					last_time = _tick_count;
					draw_order.loop_step = 0;
...
void DrawerWatchDog(void)
{
	if(draw_order.order_list.bit.watchdog == true){
		...
		if(GetElapsedTickCount(draw_status.encoder_time) >= ENCODER_TIMEOUT)
		{
			if(draw_order.order_location == DRAW_PARAM_CLOSE){
				draw_status.status_list.bit.not_closed = true;
...
void BuzzerProc(void)
{
	...
	if(draw_status.status_list.bit.not_closed){
		switch(step){
			case 0:
				if(GetElapsedTickCount(last_time) >= 1500){
					last_time = _tick_count;
					BUZZER_ON();
					step = 1;
				}
				break;
			case 1:
				if(GetElapsedTickCount(last_time) >= 2){
					last_time = _tick_count;
					BUZZER_OFF();
					step = 0;
				}
